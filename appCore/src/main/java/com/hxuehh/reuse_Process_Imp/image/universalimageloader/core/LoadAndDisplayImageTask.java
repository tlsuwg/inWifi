...
import com.hxuehh.reuse_Process_Imp.image.universalimageloader.core.assist.ImageScaleType;
import com.hxuehh.reuse_Process_Imp.image.universalimageloader.core.assist.LoadedFrom;
import com.hxuehh.reuse_Process_Imp.image.universalimageloader.core.assist.ViewScaleType;
import com.hxuehh.reuse_Process_Imp.image.universalimageloader.core.decode.ImageDecoder;
import com.hxuehh.reuse_Process_Imp.image.universalimageloader.core.decode.ImageDecodingInfo;
...
import com.hxuehh.reuse_Process_Imp.image.universalimageloader.utils.IoUtils;
	...
	private boolean checkTaskIsNotActual() {
		...
		if (imageViewWasReused) {
			handler.post(new Runnable() {
				@Override
				public void run() {
					listener.onLoadingCancelled(uri.url, imageView);
				}
			});
	...
	private Bitmap tryLoadBitmap() {
		...
		try {
			if (imageFile.exists()) {
				...
				bitmap = decodeImage(new DealUrl(ImageDownloader.Scheme.FILE.wrap(imageFile.getAbsolutePath())));
			}
...
//                String imageUriForDecoding = is ? tryCacheImageOnDisc(imageFile) : uri;
	...
	private File getImageFileInDiscCache() {
		...
		File imageFile = discCache.get(uri.url);
		File cacheDir = imageFile.getParentFile();
		if (cacheDir == null || (!cacheDir.exists() && !cacheDir.mkdirs())) {
			imageFile = configuration.reserveDiscCache.get(uri.url);
	...
	private Bitmap decodeImage(DealUrl imageUri) throws IOException,OutOfMemoryError {
	...
	private String tryCacheImageOnDisc(File targetFile) {
		...
		try {
			...
			configuration.discCache.put(uri.url, targetFile);
...
//            LogUtil.w(e);
	...
	private boolean downloadSizedImage(File targetFile, int maxWidth, int maxHeight) throws IOException,OutOfMemoryError {
		...
		if (!Thread.interrupted()) {
			handler.post(new Runnable() {
				...
				public void run() {
					...
					listener.onLoadingFailed(uri.url, imageView, new FailReason(failType, failCause));
				}
			});
		}
	}

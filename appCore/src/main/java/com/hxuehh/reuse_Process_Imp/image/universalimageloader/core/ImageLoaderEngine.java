...
class ImageLoaderEngine {
	...
	private void initExecutorsIfNeed() {
		if (!configuration.customExecutor && ((ExecutorService) taskExecutor).isShutdown()) {
			taskExecutor = createTaskExecutor("taskExecutor");
		}
		if (!configuration.customExecutorForCachedImages && ((ExecutorService) taskExecutorForCachedImages).isShutdown()) {
			taskExecutorForCachedImages = createTaskExecutor("taskExecutorForCachedImages");
		}
	}

	private Executor createTaskExecutor(String name) {
		return DefaultConfigurationFactory.createExecutor(configuration.threadPoolSize, configuration.threadPriority, configuration.tasksProcessingType,name);
	}
	 ...
	 * {@link com.hxuehh.reuse_Process_Imp.image.universalimageloader.core.assist.ImageLoadingListener#onLoadingFailed(String, android.view.View, com.hxuehh.reuse_Process_Imp.image.universalimageloader.core.assist.FailReason)} callback will be fired with
	 * {@link com.hxuehh.reuse_Process_Imp.image.universalimageloader.core.assist.FailReason.FailType#NETWORK_DENIED}
	 *
	...
	/**
	 * Sets option whether ImageLoader will use {@link com.hxuehh.reuse_Process_Imp.image.universalimageloader.core.assist.FlushedInputStream} for network downloads to handle <a
	 * href="http://code.google.com/p/android/issues/detail?id=6066">this known problem</a> or not.
	 *
	 * @param handleSlowNetwork pass <b>true</b> - to use {@link com.hxuehh.reuse_Process_Imp.image.universalimageloader.core.assist.FlushedInputStream} for network downloads; <b>false</b>
	 *                          - otherwise.
	 */

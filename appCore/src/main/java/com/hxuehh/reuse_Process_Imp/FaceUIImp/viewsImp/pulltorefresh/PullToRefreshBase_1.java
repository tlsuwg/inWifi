...
public abstract class PullToRefreshBase_1<T extends View> extends LinearLayout {
    ...
    public PullToRefreshBase_1(Context context, int mode) {
        super(context);
        this.mode = mode;
        init(context, null);
    }

    public PullToRefreshBase_1(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }
    ...
    public final void setHeaderRefreshing() {
        if (!isRefreshing()) {
            ...
            smoothScrollTo(-headerHeight-titleHeight );
...
//        if (footerLayout != null) footerLayout.setPullRefreshTime(updateAt.toString());
    ...
    protected abstract void dumpGone(boolean isGone,boolean hand);

...
public class Req extends Dealer
{
    ...
    //  If true, we are starting to sendSyncAddCache/recv a message. The first part
    ...
    public boolean xsend(Msg msg)
    {
        ...
        //  we can't sendSyncAddCache another request.
        if (receivingReply) {
    ...
    protected Msg xrecv()
    {
        //  If request wasn't sendSyncAddCache, we can't wait for reply.
        if (!receivingReply) {
            errno.set(ZError.EFSM);
            return null;
        }

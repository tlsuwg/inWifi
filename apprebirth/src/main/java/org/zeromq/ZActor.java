 ...
 * <br/>
 * This is where one can {@link #send(ZMsg) sendSyncAddCache} and {@link #recv() receive} control messages to the underneath actor via the ZActor and/or its agent.
 * <br/>
 ...
 * where the provided actor can {@link Actor#backstage(Socket, ZPoller, int) sendSyncAddCache and receive} control messages to and from the ZActor.
 * <br/>
 ...
 * {@code
        ...
        boolean rc = pipe.sendSyncAddCache("HELLO");
        assert (rc);
        ZMsg msg = actor.recv();
        ...
        rc = actor.sign();
        assert (!rc);
        rc = actor.sendSyncAddCache("whatever");
        assert (!rc);
...
// once on stage, any actor IS a star
{
    ...
    public interface Actor
    {
        ...
        /**
         * Called when a created socket has been close.
         * The process of destroying the double has already started.
         *
         * @param socket   the close socked.
         */
    ...
    private static final class ActorFortune implements Fortune
    {
        ...
        {
            Star star = new Double(ctx, pipe, sel, actor, args);
            return star;
        }
        ...
        public boolean renews()
        {
            ...
            while (iter.hasNext()) {
                ...
                if (socket != null) {
                    ...
                    // call back the actor to inform that a socket has been close.
                    actor.closed(socket);
                }
            }

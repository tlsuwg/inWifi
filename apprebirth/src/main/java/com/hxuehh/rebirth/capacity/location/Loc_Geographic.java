package com.hxuehh.rebirth.capacity.location;
 ...
 */
public class Loc_Geographic extends DeviceCapacityBase {
    ...
    @Override
    public int getType() {
        return DeviceCapacityBase.Type_Loc_Geographic;
    }
    ...
    public DeviceCapacityOutResult_3 doChangeStatus(DeviceCapacityInParameter t) throws FaceException {
        ...
        if ( !(t instanceof Loc_GeographicParameter)) throw new FaceException("参数类型出错");
        ...
        if (tt.getType() == 0) {//启动 m没有什么作用了  因为早就启动了
            if (!isInit) {
                onCreat();
            }
            if (mLocationClient != null)
                mLocationClient.requestLocation();
        } else if (tt.getType() == -1) {//关闭
            onDestry();
        } else if (tt.getType() == 2) {//最后一次 定位
            if (!isInit) {
                onCreat();
            }
            if (mLocationClient != null)
...
//                501～700：key验证失败，请按照说明文档重新申请KEY。
        ...
        public void onReceiveLocation(BDLocation location) {
            ...
            sb.append(DeviceUtil.getBrand()+""+ DeviceUtil.getimei());
            sb.append("time : ");
...
//                Log.i("BaiduLocationApiDem", sb.toString());
    ...
    public static class Loc_GeographicParameter extends DeviceCapacityInParameter implements Serializable {
        ...
        public static final int Type_time_get=3;

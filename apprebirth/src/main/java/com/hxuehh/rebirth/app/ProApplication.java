...
import com.hxuehh.rebirth.push.MiPush.MIPushMessageReceiver;
...
//                ForceCloseHandler.getInstance().init(instance);
    ...
    public DeviceService_TCPLongLink_ getTCPLongLink_DeviceService() {
        Service mService = srvicesMap.get(IntentChangeManger.Device_Service_UDPreviceback_TCPLongLink);
        if (mService == null) return null;
        return (DeviceService_TCPLongLink_) mService;
    }

    public ClientService_TCPLongLink_ getTCPLongLink_ClientService() {
        Service mService = srvicesMap.get(IntentChangeManger.Client_Service_UDPreviceback_TCPLongLink);
        if (mService == null) return null;
        return (ClientService_TCPLongLink_) mService;
    }
    ...
    private void initMiPUSH() {
        ...
        LoggerInterface newLogger = new LoggerInterface() {
            ...
            public void setTag(String tag) {
                Su.logPush("setTag" + tag);
                // ignore
            }
            @Override
            public void log(String content, Throwable t) {
                Su.logPush("setTag" + content + " " + t);
            }
